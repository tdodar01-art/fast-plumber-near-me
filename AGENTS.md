...
- **VERTICAL** (stays in `apps/fast-plumber-near-me`): plumbing-specific Codex prompts, plumbing service list, plumbing FAQ content, plumbing cost ranges, plumbing brand copy.
...
- **`daily-scrape.yml`** (6 AM Central) — GSC expansion → Places scrape → Firestore upload → review refresh → Codex synthesis → commit → Vercel rebuild
- **`deep-review-pull.yml`** (7 AM Central) — BBB lookup → Outscraper multi-source reviews (Google + Yelp + Angi) → Codex synthesis → JSON export → commit → Vercel rebuild
...
Full reference: `~/code/control-center/AGENTS.md` and `STRUCTURE.md`.
